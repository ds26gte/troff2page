...
(defun read-bare-word ()
  (let ((read-escape-p nil) (bracket-nesting 0) (r '()) c)
    (loop
      (setq c (snoop-char))
...
(defun table-do-rows ()
  (let ((*inside-table-text-block-p* nil)
    ...
    (loop
      (block next-iter
        (setq c (snoop-char))
        (cond ((= *table-cell-number* 0)
               (cond ((char= c *control-char*)
                      (get-char)
                      (let ((w (read-word)))
                        (cond ((not w))
                              ((string= w "TE") (return))
                              ((string= w "TH") (setq *reading-table-header-p* nil) (return-from next-iter))
                              (t (toss-back-string w))))
                      (toss-back-char *control-char*))
                     ((member c '(#\_ #\- #\=) :test #'char=)
                      (read-troff-line)
                      (emit-verbatim "<tr><td valign=top colspan=")
                      (emit-verbatim *table-number-of-columns*)
                      (emit-verbatim "><hr></td></tr>")
                      (emit-newline)
                      (return-from next-iter)))
               (table-do-cell))
...
(defun emit (s)
  (with-input-from-string (i s)
    (let ((inside-html-angle-brackets-p nil) it c)
      (loop
        (setq c (read-char i nil))
        (when (not c) (return))
        (ecase *outputting-to*
          ((:html :title)
...
(defun eval-in-lisp (ss)
  (let ((s (with-output-to-string (o)
             (dolist (x ss)
               (princ x o) (terpri o)))))
    (with-input-from-string (i s)
      (let ((*package* (find-package :troff2page)) x)
        (loop
          (setq x (read i nil))
          (when (not x) (return))
          (eval x))))))
...
(defun split-string-at-char (p c)
  (if (not p) '()
      (let ((r '()) (start 0) i)
        (loop
          (setq i (position c p :start start :test #'char=))
          (when (not i)
